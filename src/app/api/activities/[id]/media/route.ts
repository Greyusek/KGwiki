...
import { addActivityExternalLink, addActivityMedia } from "@/services/activity-service";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (contentType.includes("application/json")) {
    ...
    if (!body.externalUrl) return NextResponse.json({ error: "URL is required." }, { status: 400 });
    const isValid = /^https?:\/\//i.test(body.externalUrl);
    if (!isValid) return NextResponse.json({ error: "URL must start with http or https." }, { status: 400 });
    const result = await addActivityExternalLink(id, { title: body.title.trim(), externalUrl: body.externalUrl, description: body.description }, { id: session.user.id, role: session.user.role });
  ...
  const result = await addActivityMedia(id, { fileName: file.name, objectPath, mimeType: file.type, fileSize: file.size }, { id: session.user.id, role: session.user.role });
  if (!result.ok) return NextResponse.json({ error: result.error }, { status: result.status });
  return NextResponse.json({ data: result.media }, { status: 201 });
}
